...
public partial struct SpawnTombstoneSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        var arrayBuilder = builder.Allocate(ref spawnPoints.Value, graveyard.NumberOfThumbstones);

        for (var i = 0; i < graveyard.NumberOfThumbstones; i++)
        {
            var newTombstone = ecb.Instantiate(graveyard.TombstonePrefab);
            var newTombstoneTransform = graveyard.GetRandomThumbstoneTransform();
